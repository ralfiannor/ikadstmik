...
if(isset($_POST['btn-save']))
{
    ...
    else
    {
        ...
        {
            redirect(site_url('dosen/ganti-sandi/?sukses'));
        }
        else
        {
            redirect(site_url('dosen/ganti-sandi/?gagal'));
        }
...
if (isset($_GET['password_lama'])) {
    ...
    $stmt = $login->runQuery("SELECT id,nidn,password FROM dosen WHERE id=:uname");
    $stmt->execute(array(':uname'=>$dosen_id));
    ...
    <div class="grid-form1">    
        <h3>Ganti Kata Sandi</h3><br>
            ...
            <div class="form-group has-feedback">
                ...
                <input type="password" name="password_lama" class="form-control" <?= (isset($_GET['edit_id'])) ? '' : 'data-remote="'.site_url('dosen/ganti-sandi/').'" data-error="Kata sandi tidak benar"' ?> maxlength="155" data-required-error="Kata sandi tidak boleh kosong" required>
            ...
            <a href="<?= site_url('dosen/ganti-sandi') ?>" class="btn btn-large btn-success"><i class="glyphicon glyphicon-backward"></i> &nbsp; Batal</a>
        </form>
    </div>
</div>
