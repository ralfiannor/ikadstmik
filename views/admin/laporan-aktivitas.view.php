...
<?php $laporanaktivitas->cetak($tahun_akademik,$semester,$jurusan); ?>
<div class="footer">
    <table border="0" width="850" style="margin-right: 0%;margin-left: 10%">
        <tr>
...
<div class="grid-form">
    <div class="grid-form1">
        <h3 id="grid-example-basic">Laporan aktivitas Dosen</h3>

            <form method="get" action="<?= site_url('admin/laporan-aktivitas/')?>" data-toggle="validator" role="form" data-feedback='{"success": "fa-check", "error": "fa-times"}' id="formdata">
...
$tahun_akademik = $_GET['tahun1']."/".$_GET['tahun2'];
$semester = $_GET['semester'];
                $query = "SELECT dosen_ampu.id, matakuliah.kd_mk, matakuliah.nama_mk, matakuliah.sks, matakuliah.semester, matakuliah.jurusan, dosen.nidn, dosen.nama FROM aktivitas_dosen as a INNER JOIN dosen_ampu ON a.id_dosenampu = dosen_ampu.id INNER JOIN dosen ON dosen_ampu.nidn = dosen.nidn INNER JOIN matakuliah ON dosen_ampu.kd_mk = matakuliah.kd_mk WHERE a.tahun_akademik = '".$tahun_akademik."' AND a.semester = '".$semester."' GROUP BY a.id_dosenampu";  
                $laporanaktivitas->laporandosen($query,$tahun_akademik,$semester);
}
else {
